...
## The TransactionListByVendor Report Object
...
| Attribute | Description |
|-----------|-------------|
| ReportName | Name of the report (TransactionListByVendor) |
...
| Amount | nat_amount | Transaction amount |
...
```
GET /v3/company/<realmID>/reports/TransactionListByVendor?<name>=<value>[&...]
```
...
- last_mod_date, po_status, ship_via, olb_status, vendor_name
...
```
GET https://quickbooks.api.intuit.com/v3/company/1386066315/reports/TransactionListByVendor?start_date=2016-06-01&end_date=2016-07-31&group_by=Vendor
```
...
```json
{
  ...
  "Columns": {
    "Column": [
      ...
      {"ColTitle": "Amount", "MetaData": [{"Name": "ColKey", "Value": "nat_amount"}]}
    ]
  }
}
```
...
- This report is vendor-focused, showing all transactions related to vendors
- Useful for accounts payable analysis and vendor payment tracking
- Can be grouped by vendor to see all transactions per vendor
- Supports filtering by AP paid status (Paid, Unpaid, All)
